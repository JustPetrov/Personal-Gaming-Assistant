...
@app.get("/api/news")
def latest_news():
    """Return the newest generated news/update article for the homepage."""
    candidates = []
    for filename in ("news.json", "news_update.json", "latest_news.json", "updates.json"):
        value = read_json(filename, [])
        if isinstance(value, dict):
            value = value.get("items") or value.get("updates") or value.get("news") or [value]
        if isinstance(value, list):
            candidates.extend(x for x in value if isinstance(x, dict))
    if not candidates:
        return {"title": "Nog geen nieuwsupdate", "summary": "De eerste nieuwsupdate wordt tijdens de monitoringcyclus aangemaakt.", "timestamp": None, "source": None, "url": None}
    def stamp(item):
        return str(item.get("published_at") or item.get("timestamp") or item.get("created_at") or item.get("updated_at") or "")
    newest = max(candidates, key=stamp)
    return {
        "title": newest.get("title") or newest.get("headline") or newest.get("name") or "Nieuwsupdate",
        "summary": newest.get("summary") or newest.get("description") or newest.get("content") or newest.get("text") or newest.get("message") or "",
        "timestamp": newest.get("published_at") or newest.get("timestamp") or newest.get("created_at") or newest.get("updated_at"),
        "source": newest.get("source") or newest.get("publisher"),
        "url": newest.get("url") or newest.get("link"),
    }
...
<main>
<section class="card wide news"><h2>Laatste nieuwsupdate</h2><div id="news">Laden...</div></section>
